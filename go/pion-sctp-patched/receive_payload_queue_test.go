...
func TestReceivePayloadQueue(t *testing.T) {
	...
	assert.False(t, ok)
	assert.Empty(t, payloadQueue.getGapAckBlocks(payloadQueue.chunkSize))
	...
	gaps := payloadQueue.getGapAckBlocks(payloadQueue.chunkSize)
...
func TestReceivePayloadQueueAdvanceCumulativeTSN(t *testing.T) {
	...
	assert.EqualValues(t, []gapAckBlock{{start: 15, end: 15}}, payloadQueue.getGapAckBlocks(payloadQueue.chunkSize))
